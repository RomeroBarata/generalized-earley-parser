...
def read_features(segments_feature_path, filename):
    ...
    with open(filename) as f:
        ...
        # Why they have the zeros in the 160:165 and 795:800?
    ...
    if segment_index == 1:
        ...
        temporal_file = os.path.join(segments_feature_path,
...
def expand_data(paths, features, gt_labels):
    ...
    for datadir in os.listdir(os.path.join(paths.cad_data_root)):
        ...
        if os.path.isdir(datadir) and datadir.endswith('annotations'):
            for subjectdir in os.listdir(datadir):
                subject = os.path.split(subjectdir)[1].strip('_annotations')
                data = dict()
                subjectdir = os.path.join(datadir, subjectdir)
                for event in os.listdir(subjectdir):
                    eventdir = os.path.join(subjectdir, event)
...
def collect_data(paths):
    ...
    # segments_files_path = os.path.join(paths.cad_data_root,
    #                                    'features_cad120_ground_truth_segmentation',
    #                                    'segments_svm_format')
    # segments_feature_path = os.path.join(paths.cad_data_root,
    #                                      'features_cad120_ground_truth_segmentation',
    ...
    for sequence_path_file in os.listdir(segments_files_path):
        ...
        with open(os.path.join(segments_files_path, sequence_path_file)) as f:
            ...
            for _ in range(segment_feature_num):
                ...
                filename = os.path.join(segments_feature_path, os.path.basename(segment_feature_filename))
                human_features, gt_label = read_features(segments_feature_path, filename=filename)
